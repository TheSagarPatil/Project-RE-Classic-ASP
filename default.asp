	...
	%>
		<div class="flex-cards col-xs-12 col-sm-6 col-md-4 col-xs-12">
			<div class="jumbotron pricing-card card">
				...
				<div class="row">
					<div class="imageHolder" style="background:url('images/props/<%Response.Write(rs.Fields(0))%>.jpg'); background-size:cover;background-position: center;">
					</div>
				</div>
				<div class="row">
				...
				<div class="row">
					<div class="col-xs-12">
						...
						<%if Session("access") = "cust" then%>
						<form name="f_inquire<% Response.Write(rs.Fields(0)) %>" action="action_insertInquiry.asp">
							<input type="hidden" name="prop" value="<% Response.Write(rs.Fields(0)) %>"/>
							<input type="hidden" name="cust" value="<% Response.Write(Session(customerID)) %>"/>
							<input type="hidden" name="rdrPath" value=" <%Response.Write(Request.ServerVariables("SCRIPT_NAME")) %>">
							<%if Session("access") = "cust" then%>
							<button class="btn btn-success btn-choose-basket col-xs-12"  submit>Inquire</button>
							<% end if %>
