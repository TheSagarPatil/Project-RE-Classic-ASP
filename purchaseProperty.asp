...
<%
propImgId = request.querystring("prop")
if Session("access")="cust" and not Session("customerId") = "" then
%>
	<div class="container containerMain">
		
		<form name="purchaseForm" method="get" action="purchaseReciept.asp">
		<div class="col-xs-12 col-lg-10 col-lg-offset-1" style="padding:0px 24px 16px 24px; border-radius:2px; box-shadow: 0px 5px 10px 2px ;">
			...
			<div class="row">
				<div class="col-xs-6 ImageHolderLarge" style="height:250px; margin-bottom:16px;overflow:hidden; background-image: url('images/props/<%Response.Write(propImgId)%>.jpg');background-size:cover;">
				</div>
				<div class="col-xs-6">
					<div class="row">
						<div class="col-xs-12">
					...
					<div class="row">
						<div class="col-xs-12">
							...
							<input type="hidden" name="prop"value="<%Response.Write(request.querystring("prop"))%>"/>
							</div>
						</div>
					</div>
					<div class="row">
	...
	*/
	...
	$('[name="flatId"]').html("");
	$('[name="flatId"]').append("<option value='0'>Select 1</option>");
